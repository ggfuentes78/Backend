...
const {buscaUsuario, altaUsuario, buscaUsuarioPorId} = require('../models/users/users.mongo.dao');
...
const validarLogin=(ctx, next)=>{
    if (ctx.isAuthenticated()){
        user=ctx.request.session.user;
        next()
    }else{
        ctx.render('login')
    }
...
const login = async(ctx, username, password, done)=>{
...
const signup = async (ctx, username, password, done)=>{
    try{
        logger.info('intentando registro de usuario...');
        const { email, password, admin } = ctx.request.body;
